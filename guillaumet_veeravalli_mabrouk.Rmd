...
```{r}
country <- levels(as.factor(life_table_pivot$Country))

scree_male<- data.frame()
scree_female<- data.frame()
for (i in country) {
  pca_male <- life_table_pivot %>% filter(Country == i, Gender == "Male", Year %in% 1948:2010 )
  pca_male <- prcomp(pca_male[,3:113],scale.=TRUE, center=TRUE)
  pca_female <- life_table_pivot %>% filter(Country == i, Gender == "Female", Year %in% 1948:2010 )
  pca_female <- prcomp(pca_female[,3:113],scale.=TRUE, center=TRUE)
}
var_explained_df <- data.frame(PC= paste0("PC",1:5),
                               var_explained=(pca_male$sdev[1:5])^2/sum((pca_male$sdev[1:5])^2))
 
cbind(t(var_explained_df) 
