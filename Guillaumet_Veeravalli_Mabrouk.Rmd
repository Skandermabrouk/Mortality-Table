...
```
# cOMPARE WITH SVD RANK 2 USING SCHMIDT NORM (USA BOTH)
```{r}
res_USB <- svd(A_both)
...
```
#COMPARE WITH RANK 2 SVD (USA FEMALE)
```{r}
res_USF <- svd(A_F)
...
```
#COMPARE WITH RANK 2 SVD (USA MALE)
```{r}
res_USM <- svd(A_M)
...
```
#COMPARE WITH RANK 2 SVD (FRANCE BOTH)
...
```
#COMPARE WITH RANK 2 SVD (FRANCE MALE)
```{r}
res_FRM <- svd(A_FRM)
...
```
#COMPARE WITH RANK 2 SVD (FRANCE FEMALE)
```{r}
res_FRF <- svd(A_FRF)
...
#Predictions of life expectancies at different ages - France Female

#GETTING EX PIVOT FROM LIFE TABLE
```{r}
...
```{r}
#PLOTTING Theorical from prediction/Observed
prediction_observed_ex_fr_female <- ex_observed_pivot_fr_female %>% ggplot() +
  aes(x = Year, y = ex_observed_pivot_fr_female[,4]) +
  geom_line(col = "red") +
  geom_line(data = ex_predicted_pivot_fr_female, y = ex_predicted_pivot_fr_female[,4], col = "blue") +
  #geom_line(data=ex_observed_pivot_fr_female, x = Year, y = ex_observed_pivot_fr_female[,4], col = "red") +
  labs(y="Age", title = "Prediction of life expectancy at given age")

prediction_observed_ex_fr_female
```
