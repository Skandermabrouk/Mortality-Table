...
pcafemme<-PCA_F %>%
  ...
  ggtitle("Scree plot: Share of inertia standarisation ")+
...
pcahomme<-PCA_M %>%
  ggplot(aes(x=PC,y=var_explained,fill=Country))+
  geom_col(position = "dodge")+
  ggtitle("Scree plot: Share of inertia Avec standarisation")+
...
pcafemme2<-PCA_F_2 %>%
  ggplot(aes(x=PC,y=var_explained,fill=Country))+
  geom_col(position = "dodge")+
  ggtitle("Scree plot: Share of inertia without standarisation ")+
  labs(subtitle = "per Country Female")+
  theme(axis.text.x=element_text(angle=-45, hjust=0, vjust=1)) +ylab("Percent")+xlab("Principal Componement") +
  theme(plot.title=element_text(size=12, hjust=0.5, face="bold", colour="maroon", vjust=-1)) +
   theme(plot.subtitle=element_text(size=10, hjust=0.5, face="italic", color="black"))
...
svdUS_M_SansCarter <- svd(A_M,2,2)
u1<- -1*svdUS_M_SansCarter$u
u2<- -1*svdUS_M_SansCarter$v

norm(u1[,1],type = "F")
norm(u_USmSVD, type = "F")

norm(u2, type = "F")
norm(v_USmSVD, type= "F")

norm(svdUS_M_SansCarter$d, type = "F")
...
#esssayer d'avoir une matrice de la meme dim que life table pivot filtre aux USA 
pivot_life_lx <- function(life_table) {
pivot_life_lx <- life_table %>%
  select(Country, Gender, Year, Age, lx) %>%
    pivot_wider(
...
```{r}
# ax est constant on compare uniquement le kappa et beta de deux SVD 
#on utilise la fonction norm() de R
```
