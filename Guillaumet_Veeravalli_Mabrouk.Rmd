...
Calcul de l'esperance de vie à la naissance d'une population selon une année A;
...
#ALGORITHME TRES GOURMAND EN RESSOURCES DONC ON A SAUVEGARDER LE RESULTAT EN .Rds
...
#CHOIX DU PCA CENTER = TRUE, SCALE
...
pacman::p_load(lifecontingencies)
```
...
```
# cOMPARE WITH SVD RANK 2 USING SCHMIDT NORM
```{r}

res2_f=svd(usa_f_df)
A2_f=(res2_f$u[,1:2])%*%diag(res2_f$d[1:2])%*%t(res2_f$v[,1:2])
mat_f=matrix(0,nrow=63,ncol=110) #mat modele lee carter
for(i in 1:110){               #A2 modele svd tronqué rang 2
  mat_f[,i]=a_f[i]+b_f[i]*k_f
}
norm(mat_f-A2_f,"F") #Question 2
norm(as.matrix(usa_f_df)-mat_f) #pour obtenir les epsilons

#Lee Carter
usa_m_df<-life_table_pivot%>%subset(Country=="USA"& Gender=="Male" & Year<1996,select=c(4:113))%>%log()
a=colMeans(usa_m_df)
id=matrix(1,nrow=63,ncol=1)
res=usa_m_df-(id%*%t(a))
res
svd(res,1,1)
s=svd(res,1,1)$d[1]
...
b=svd(res,1,1)$v
res2=svd(usa_m_df)
A2=(res2$u[,1:2])%*%diag(res2$d[1:2])%*%t(res2$v[,1:2])
mat=matrix(0,nrow=63,ncol=110)
for(i in 1:110){
  mat[,i]=a[i]+b[i]*k
}
norm(mat-A2,"F") #Question 2
norm(as.matrix(usa_m_df)-mat)
#question 4
k2=cbind(k,c(1933:1995))
reg=lm(k2[,1]~k2[,2])
as.data.frame(summary(reg)$coefficients)[1,1]->intercept
as.data.frame(summary(reg)$coefficients)[2,1]->slope
k_fitted=matrix(0,nrow=16,ncol=2)
k_fitted[,1]=c(2000:2015)
k_fitted[,2]=intercept+slope*k_fitted[,1]
plot(k2[,2],k2[,1])
plot(x=k_fitted[,1],y=k_fitted[,2])
#Question 5
mat_log=matrix(0,nrow=16,ncol=110)
for(i in 1:110){
  mat_log[,i]=a[i]+b[i]*k_fitted[,2]
}
mat_log %>% exp() %>% as.matrix() -> mat_exp
life_table_pivot %>% subset(Country=="USA"& Gender=="Male" & Year%in%c(2000:2015),select=c(4:113)) %>% as.matrix() ->dfusa2000
norm(mat_exp-dfusa2000,"F")
mat2=rbind(mat_exp[1,],mat_exp[6,],mat_exp[11,],mat_exp[16,])
abs(mat_exp[1,1]-dfusa2000[1,1])
...
g11 <- ggplot(data_1) + geom_point(aes(age,a1)) +
  ...
  theme_bw() +
  ggtitle("Alpha estimating of USA  Both DEMOGDATA") +
  labs(y = bquote(hat(alpha)[x]^"(1)"))
#beta
g12 <- ggplot(data_1) + geom_point(aes(age,b1)) +
  geom_line(aes(age,b1), col = "red") +
  theme_bw() +
  ggtitle("Beta estimating of USA Both DEMOGDATA") +
  labs(y = bquote(hat(beta)[x]^"(1)"))
...
g13 <- ggplot(data_period_USM) + geom_point(aes(year,k1)) +
  ...
  theme_bw() +
  ggtitle("kappa estimating of USA  Both DEMOGDATA") +
  labs(y = bquote(hat(kappa)[t]^"(1)"))
...
#LEE CARTER DEMOGDATA USA FEMALE
```{r,warning=FALSE,include=FALSE}
...
```{r}
#refaire un fit a partir les annees 1900 : 1990 
```
...
```{r}
rf2<-demography::life.expectancy(FRfemale)%>%
  tibble() 

rf<-Residual_Exp_life_table%>%
  filter(Country == "France"  ,Age %in% 1:100, Gender == "Female") %>%
  select( Year , res_lex)

exx <- life_table %>%
  filter(Country == "France" , Gender == "Female", Year %in% 1933:1995)
exxx <- exx$ex %>%
  tibble()
...
plot(rf2)
rf2 %>%
  ggplot(aes(x = res_lex_carter))

```
```{r}
life_table %>% subset(Year%in%seq(2000,2015,5) & Country == "USA" & Gender == "Male", select=c(Year,Age,qx))->df10
qx_fitted=c(1:440)
for(i in 1:4){
  for(j in 1:110){
    qx_fitted[(i-1)*110+j]=mat2[i,j]
  }
}
qx_fitted = as.data.frame(qx_fitted)
cbind.data.frame(df10,qx_fitted) -> df10
df10%>% ggplot(mapping = aes(x=jitter(Age),y=jitter(qx)))+
  geom_point(size=1,alpha=0.6)+
  geom_point(data=df10, aes(x=Age, y=qx_fitted,color="blue")) +
  facet_wrap(vars(Year))
#Question life expectancy predict
esp_vie=function(qx,age){
  mx=qx/(1-qx/2)          #On calcule le vecteur de taux de mortalité mx en fonction de qx
  return(sum(cumprod(as.numeric(1-mx[(age+1):length(mx)]))))
}
df_esp=function(df){                              # /!\ La fonction met à peu près 1 minute à s'éxécuter.
  res.lex=c(1:dim(df)[1])
  for (i in 1:((dim(df)[1])/110)){
    for (j in 1:110){
      res.lex[((i-1)*110)+j]=esp_vie(df$qx[((i-1)*110+1):(i*110)],(j-1))
    }
  }
  df%>%
    select(Year,Age)->m
...
df11=df_esp(df10)
life_table%>%subset(Year%in%seq(2000,2015,5)&Gender=="Male"&Country=="USA",select=(c(ex)))->ex
table((abs(df11$res.lex-ex))<0.5)
```
