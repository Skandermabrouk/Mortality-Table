...
norm(matLEEFRB -as.matrix(A_FRB), "F") #23.88194
```
...
#  filter(Country == "France"  ,Age %in% 1:100, Gender == "Female", Year %in% 1933:1995) %>%
#  select( Year , res_lex)



#lf =life.expectancy(FRfemale, series = names(FRfemale$rate)[1], years = FRfemale$year,
#   age = min(FRfemale$age),
#  max.age = min(100, max(FRfemale$age)))
#lf[118:180] %>%
#  tibble()
...
#residuals life of lee carter & observations 


alphhh = LCforFRB$fitted
t(alphhh)
c <- c()
for (i in 1:100){
  a=ex_res(t(alphhh),i)
...
al =cbind ("an" =rep(1933:1995,each=100),al)

colnames(al)[2]<-"res_Lee"


al %>%
  ggplot(aes(Year , res_Lee) ) +
  geom_point(col = as.factor(levels(al$an)))
