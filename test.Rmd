...
#reshaping the life table given to a demogdfata
...
uSA <-pivot_life_table %>%
  dplyr::filter(Country == 'USA' , Gender == 'Both' ,Year %in% 1933:1995)%>%
...
lx<- l__x %>%
  filter(Country == 'USA' , Gender == 'Both' , Year %in% 1933:1995) %>%
...
plot(forecast(ARIMA,h=15))

LCH0 <- lca(USm,years=1948:2005)
...
##############"""
  ...
  test1 <-life_table_pivot %>%
  filter (Country == 'USA' , Gender == 'Both' , Year %in% 1933:1995) %>%
  select(-Year,-Country,-Gender)

#testA => 63*110

test1 <- log(test1)
a_hat <- rowMeans(test1)
Z <- test1 - a_hat
...
d_square <- d^2
proportion_variance <- max(d)^2 / sum(d_square)
...
m<- filter(pivot_life_table , Country == 'USA' ,Gender == 'Both', Year %in% Years) %>%
  select(-Gender,-Country,-Year)
m<- log(m)
n.x = nrow(m) # 63
n.y = ncol(m) # 110

m_mat <- as.matrix(m, nrow = n.x, ncol = n.y) # 110 X 63
...
#m_hat <- t(m_mat)
for(i in 1:110) m_hat[,i] <- m_mat[,i] - a_age[i]
m_hat <- m_hat[,1:110]
age_mod <- 1:110
#SVD
svd_ <- svd(m_hat,1,1)
...
#lee carter svd for european data 
#we choose as an example the 


```{r}


france_Male<-pivot_life_table %>%
  filter(Country == 'France', Gender == 'Male')

france_female<-pivot_life_table %>%
  filter(Country == 'France', Gender == 'Female')

min_year_female <- min(france_female$Year) #1816
min_year_male <- min(france_Male$Year) #1816
max_year_female <- max(france_female$Year) #2017
max_year_male <- max(france_Male$Year) #2017


years_male = min_year_male:max_year_male
years_female = min_year_female:max_year_female


#creating hat mat male & female for all ages and all years 

france_female <- france_female %>%
  select(-Gender,-Country,-Year) #202 x 110

france_Male <- france_Male %>%
  select(-Gender,-Country,-Year) #202 * 110

m_hat_male_fr <- as.matrix(france_Male)
m_hat_female_fr <- as.matrix(france_female)


a_age_male <- colMeans(m_hat_male_fr)
a_age_female <- colMeans(m_hat_female_fr)

#male svd france 
for(i in 1:110) m_hat_male_fr[,i] <- m_hat_male_fr[,i] - a_age_male[i]
m_hat_male_fr <- m_hat_male_fr[,1:110]
age_mod <- 1:110
#SVD
svd_ <- svd(m_hat_male_fr,1,1)
v <- svd_$v
u <- svd_$u
d <- svd_$d
...
b <- v/sum(v)
k <- u * sum(v) * d[1]
fit_1 = a_age_male[age_mod] + (b * k[1])
plot(x = age_mod, y = a_age_male, pch = 19, xlab = "Age", ylab = "m")
lines(x = age_mod, y = fit_1, col = 2, lwd = 2, xlab = "Age", ylab = "m")

fit_end <- a_age[age_mod] + (b * k[63])
plot(x = age_mod, y = m_hat_male_fr[63,age_mod], pch = 18)
lines(x = age_mod, y = fit_end, col = 2, lwd = 2)
...
LC_female_usa<-demography::lca(USfemale)
...
LcsimUSF <- simulate(LCfit, nsim = 1000 , h= 20)
...
#fancharts based on period index
